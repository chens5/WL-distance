...
def make_descriptor(adjacency: np.ndarray, *, q: float, k: int, label_mode: str) -> GraphDescriptor:
    ...
    return GraphDescriptor(n, degree, mu, transition, labels, label_values, pushed)
...
def main() -> None:
    ...
    parser.add_argument("--label-mode", choices=("f1", "f2"), default="f2")
    ...
    descriptors = [
        make_descriptor(adjacency, q=args.q, k=args.k, label_mode=args.label_mode)
        for adjacency in adjacencies
    ]
    mapping_audit = repo_f2_static_audit(adjacencies) if args.label_mode == "f2" else None
    distance_path = args.output_dir / f"{args.dataset_name}_wllb_{args.label_mode}_k{args.k}.npy"
